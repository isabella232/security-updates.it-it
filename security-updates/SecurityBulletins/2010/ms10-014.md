---
TOCTitle: 'MS10-014'
Title: 'Bollettino Microsoft sulla sicurezza MS10-014 - Importante'
ms:assetid: 'ms10-014'
ms:contentKeyID: 61240453
ms:mtpsurl: 'https://technet.microsoft.com/it-IT/library/ms10-014(v=Security.10)'
author: SharonSears
...
Bollettino Microsoft sulla sicurezza MS10-014 - Importante
==========================================================

Una vulnerabilità in Kerberos può consentire un attacco di tipo Denial of Service (977290)
------------------------------------------------------------------------------------------
...
Questo aggiornamento risolve la vulnerabilità correggendo il modo in cui il server Kerberos gestisce le richieste di rinnovo dei ticket. Per ulteriori informazioni sulla vulnerabilità, vedere la sottosezione Domande frequenti per la specifica voce della vulnerabilità nella sezione **Informazioni sulle vulnerabilità**.
...
|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------|------------------------------|-----------------------------------------------|
| [Microsoft Windows 2000 Server Service Pack 4](http://www.microsoft.com/downloads/details.aspx?familyid=56a9afba-a6ee-4fb9-ba85-e51d9f94de27)                                                         | Denial of Service                           | Importante                   | Nessuno                                       |
| [Windows Server 2003 Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=738e2fda-96fc-413d-a5c7-74b1fac1d6b3)                                                                   | Denial of Service                           | Importante                   | Nessuno                                       |
| [Windows Server 2003 x64 Edition Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=f8ad539d-8191-4864-977b-ad4e31c04ba0)                                                       | Denial of Service                           | Importante                   | Nessuno                                       |
| [Windows Server 2003 con SP2 per sistemi Itanium](http://www.microsoft.com/downloads/details.aspx?familyid=d549c927-add7-4d83-bfc7-15dc35663dfb)                                                      | Denial of Service                           | Importante                   | Nessuno                                       |
| [Windows Server 2008 per sistemi a 32 bit e Windows Server 2008 per sistemi a 32 bit Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=75327470-0f14-4cdd-bcb7-64684c61c267)\* | Denial of Service                           | Importante                   | Nessuno                                       |
| [Windows Server 2008 per sistemi x64 e Windows Server 2008 per sistemi x64 Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=7d0f8f81-fc10-450a-a653-06f89acba9fa)\*           | Denial of Service                           | Importante                   | Nessuno                                       |
...
L'aggiornamento sarà offerto ai sistemi su cui il componente interessato è installato e attivato. I sistemi che non ricoprono il ruolo di controller di dominio contengono il componente vulnerabile ma non sono interessati. Per questo motivo non viene loro offerto il presente aggiornamento tramite Aggiornamenti automatici.
...
<th style="border:1px solid black;" >Vulnerabilità legata alla risoluzione del riferimento di un puntatore NULL in Kerberos - CVE-2010-0035</th>
...
Vulnerabilità legata alla risoluzione del riferimento di un puntatore NULL in Kerberos - CVE-2010-0035  
------------------------------------------------------------------------------------------------------
  
<span></span>
Una vulnerabilità ad attacchi di tipo Denial of Service è stata riscontrata in alcune implementazioni di Kerberos. Tale vulnerabilità è dovuta a una errata gestione delle richieste di rinnovo del ticket di concessione ticket (TGT) inviate da un client situato in un'area di autenticazione remota non Windows nel caso di una implementazione di Kerberos in modalità mista. Sfruttando tale vulnerabilità un utente malintenzionato può impedire al controller di dominio Windows interessato di rispondere alle richieste.
  
Per visualizzare questa vulnerabilità come voce standard nell'elenco CVE, vedere il codice [CVE-2010-0035](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-0035).
  
#### Fattori attenuanti della vulnerabilità legata alla risoluzione del riferimento di un puntatore NULL in Kerberos - CVE-2010-0035
  
Per fattore attenuante si intende un'impostazione, una configurazione comune o una procedura consigliata generica esistente in uno stato predefinito in grado di ridurre la gravità nello sfruttamento di una vulnerabilità. I seguenti fattori attenuanti possono essere utili per l'utente:
  
-   Per sfruttare tale vulnerabilità, l'utente malintenzionato deve essere in possesso di credenziali di accesso valide per un'area di autenticazione considerata attendibile dal controller di dominio di Windows. La vulnerabilità non può essere sfruttata da utenti anonimi.  
-   La vulnerabilità interessa soltanto i controller di dominio. La vulnerabilità non interessa i server che non ricoprono il ruolo di controller di dominio.
  
#### Soluzioni alternative per la vulnerabilità legata alla risoluzione del riferimento di un puntatore NULL in Kerberos - CVE-2010-0035
  
Microsoft non ha individuato alcuna soluzione alternativa per questa vulnerabilità.
  
#### Domande frequenti sulla vulnerabilità legata alla risoluzione del riferimento di un puntatore NULL in Kerberos - CVE-2010-0035
  
**Qual è la portata o l'impatto di questa vulnerabilità?**    
Si tratta di una vulnerabilità ad attacchi di tipo Denial of Service. Un utente malintenzionato remoto autenticato può inviare una richiesta appositamente predisposta di rinnovo di un ticket di concessione ticket (TGT, Ticket-Granting-Ticket) esistente, il quale può impedire al sistema interessato di rispondere provocando una condizione di Denial of Service. La condizione di Denial of Service può persistere fino a quando non viene riavviato il controller di dominio.
  
**Quali sono le cause di questa vulnerabilità?**    
Questa vulnerabilità è causata da una errata gestione delle richieste di rinnovo dei ticket inviate da un dominio Kerberos non Windows.
...
**Che cos'è un ticket di concessione ticket (TGT, Ticket-Granting-Ticket)?**    
Il Centro distribuzione chiave Kerberos (KDC) assegna a un client una chiave temporanea. Tale chiave è valida solo per una determinata sessione di accesso. Quando un utente effettua l'accesso, il client richiede un ticket per il KDC, esattamente come farebbe per qualunque altro servizio. Il KDC risponde creando una chiave di sessione di accesso e un ticket per un server speciale, il servizio completo di concessione ticket del KDC. Una copia della chiave di sessione di accesso viene incorporata nel ticket e il ticket viene crittografato tramite la chiave master del KDC. Un'altra copia della chiave di sessione di accesso viene crittografata tramite la chiave master dell'utente derivata dalla password di accesso dell'utente. Sia il ticket che la chiave di sessione crittografata vengono inviati al client.
  
Quando il client riceve una risposta dal KDC, decrittografa la chiave di sessione di accesso tramite la chiave master dell'utente derivata dalla password dell'utente. Ora il client non ha più bisogno della chiave derivata dalla password dell'utente poiché può utilizzare la chiave di sessione di accesso per decrittografare la propria copia di qualunque chiave di sessione di server ottenuta dal KDC. Il client archivia la chiave di sessione di accesso nella cache del suo ticket assieme al ticket per il servizio completo di concessione ticket del KDC. Il ticket per il servizio completo di concessione ticket viene definito ticket di concessione ticket o TGT (Ticket-Granting-Ticket).
  
**A quali attacchi viene esposto il sistema a causa di questa vulnerabilità?**    
Un utente malintenzionato potrebbe sfruttare la vulnerabilità impedendo al sistema interessato di rispondere. Per ripristinare il normale funzionamento, l'amministratore deve riavviare il sistema manualmente. Anche in caso di totale indisponibilità del controller di dominio, gli utenti a cui erano stati concessi ticket Kerberos in precedenza rimarrebbero in grado di utilizzarli, continuando ad accedere a tutte le risorse per le quali dispongono di ticket. La condizione di Denial of Service impedirebbe tuttavia al controller di dominio di concedere nuovi ticket di accesso ad altre risorse.
  
**In quale modo un utente malintenzionato può sfruttare questa vulnerabilità?**    
Un utente malintenzionato che è stato autenticato in un'area Kerberos non Windows considerata attendibile dal dominio Kerberos Windows può sfruttare tale vulnerabilità inviando una richiesta di rinnovo del ticket appositamente predisposta al suo KDC Kerberos.
  
**Quali sono i sistemi principalmente interessati da questa vulnerabilità?**    
Sono interessati dalla vulnerabilità i controller di dominio che hanno definito un'area di autenticazione Kerberos in modalità mista. Le aree di autenticazione Kerberos in modalità mista sono distribuzioni nelle quali l'area di autenticazione Kerberos Windows considera attendibile una implementazione Kerberos non Microsoft di terze parti.
  
**Quali sono gli scopi dell'aggiornamento?**    
Questo aggiornamento risolve la vulnerabilità correggendo il modo in cui il server Kerberos gestisce le richieste di rinnovo dei ticket.
...
| **Distribuzione**                                  |                                                                                                                                                                   |  
| Installazione senza intervento dell'utente         | Windows2000-KB977290-x86-ENU /quiet                                                                                                                               |  
| Installazione senza riavvio                        | Windows2000-KB977290-x86-ENU /norestart                                                                                                                           |  
| Aggiornamento del file di registro                 | kb977290.log                                                                                                                                                      |  
...
| **Informazioni per la rimozione**                  | Utilizzare **Installazione applicazioni** dal Pannello di controllo o l'utilità Spuninst.exe disponibile nella cartella %Windir%\\$NTUninstallKB977290$\\Spuninst |  
| **Informazioni sui file**                          | Consultare l'[articolo della Microsoft Knowledge Base 977290](http://support.microsoft.com/default.aspx?ln=it)                                                    |  
| **Controllo della chiave del Registro di sistema** | HKEY\_LOCAL\_MACHINE\\SOFTWARE\\Microsoft\\Updates\\Windows 2000\\SP5\\KB977290\\Filelist                                                                         |
...
<td style="border:1px solid black;">Utilizzare <strong>Installazione applicazioni</strong> dal Pannello di controllo o l'utilità Spuninst.exe disponibile nella cartella %Windir%\$NTUninstallKB977290$\Spuninst</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;"><strong>Informazioni sui file</strong></td>
<td style="border:1px solid black;">Consultare l'<a href="http://support.microsoft.com/default.aspx?ln=it">articolo della Microsoft Knowledge Base 977290</a></td>
</tr>
<tr class="even">
