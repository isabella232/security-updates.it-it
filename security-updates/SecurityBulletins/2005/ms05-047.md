---
TOCTitle: 'MS05-047'
Title: 'Bollettino Microsoft sulla sicurezza MS05-047 - Importante'
ms:assetid: 'ms05-047'
ms:contentKeyID: 61240133
ms:mtpsurl: 'https://technet.microsoft.com/it-IT/library/ms05-047(v=Security.10)'
author: SharonSears
...
Bollettino Microsoft sulla sicurezza MS05-047 - Importante
==========================================================
...
-   Microsoft Windows 2000 Service Pack 4 - [Download dell'aggiornamento](http://www.microsoft.com/downloads/details.aspx?familyid=ffdb8ab7-f979-41b4-9625-ea51cd503258)
-   Microsoft Windows XP Service Pack 1 e Microsoft Windows XP Service Pack 2 - [Download dell'aggiornamento](http://www.microsoft.com/downloads/details.aspx?familyid=1559e44a-ddee-4c86-bf02-a6c3b9beee0c)
...
| Identificatori della vulnerabilità                                                                                          | Effetti della vulnerabilità                                                                        | Windows 2000 | Windows XP |
|-----------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------|--------------|------------|
| Vulnerabilità nel servizio Plug and Play - [CAN-2005-2120](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=can-2005-2120) | esecuzione di codice in modalità remota e acquisizione di privilegi più elevati sul sistema locale | importante   | importante |
...
| ID bollettino | Windows 2000 | Windows XP |
|---------------|--------------|------------|
| **MS05-039**  | Sostituito   | Sostituito |
...
<span></span>
#### Vulnerabilità nel servizio Plug and Play - CAN-2005-2120:

Nel servizio Plug and Play è presente una vulnerabilità associata alla possibilità di esecuzione di codice in modalità remota e all'[acquisizione di privilegi più elevati a livello locale](http://go.microsoft.com/fwlink/?linkid=21142) che potrebbe consentire a un utente malintenzionato autenticato di assumere pieno controllo di un sistema interessato.

#### Fattori attenuanti della vulnerabilità nel servizio Plug and Play - CAN-2005-2120:

-   In Windows XP Service Pack 2, l'utente malintenzionato deve essere in possesso di credenziali di accesso valide e riuscire a effettuare l'accesso locale al sistema per tentare di sfruttare questa vulnerabilità. La vulnerabilità non può essere sfruttata in remoto o da utenti anonimi o da utenti che dispongono di account utente standard. Tuttavia, il componente interessato è disponibile in remoto agli utenti che dispongono di autorizzazioni amministrative.
-   In Windows 2000 e Windows XP Service Pack 1, l'utente malintenzionato deve essere in possesso di credenziali di accesso valide per tentare di sfruttare questa vulnerabilità. La vulnerabilità non può essere sfruttata in remoto da utenti anonimi. Tuttavia, il componente interessato è disponibile in remoto agli utenti che dispongono di account utente standard. In alcune configurazioni, è possibile che utenti anonimi vengano autenticati come account Guest. Per ulteriori informazioni, vedere l'[advisory Microsoft sulla sicurezza 906574](http://technet.microsoft.com/security/advisory/906574).
    **Nota** Se gli aggiornamenti per la protezione forniti con il bollettino Microsoft sulla sicurezza [MS05-039](http://technet.microsoft.com/security/bulletin/ms05-039) non sono stati installati in Windows 2000, questo problema potrebbe essere sfruttato in remoto da utenti anonimi. Se invece sono stati installati, il problema è limitato agli utenti autenticati in Windows 2000. In Windows XP Service Pack 1, questa vulnerabilità è limitata agli utenti autenticati, anche senza gli aggiornamenti per la protezione forniti con il bollettino [MS05-039](http://technet.microsoft.com/security/bulletin/ms05-039).
...
#### Soluzioni alternative per la vulnerabilità nel servizio Plug and Play - CAN-2005-2120:
...
**Nota** Come menzionato nella sezione Fattori attenuanti, Windows XP Service Pack 2 è esposto a questa vulnerabilità soprattutto da parte di utenti malintenzionati collegati localmente. Le soluzioni alternative riportate di seguito sono progettate principalmente per versioni precedenti del sistema operativo che sono esposte ad attacchi basati sulla rete.
...
#### Domande frequenti relative alla vulnerabilità nel servizio Plug and Play - CAN-2005-2120:

**Qual è la portata o l'impatto di questa vulnerabilità?**

La vulnerabilità può consentire l'esecuzione di codice in modalità remota oppure l'[acquisizione di privilegi più elevati sul sistema locale](http://go.microsoft.com/fwlink/?linkid=21142). In Windows 2000 e Windows XP Service Pack 1, un utente autenticato potrebbe tentare di sfruttare questa vulnerabilità in remoto. In Window XP Service Pack 2, solo un amministratore può essere in grado di effettuare l'accesso remoto al componente interessato. Pertanto, nel sistema Windows XP Service Pack 2, la vulnerabilità potrebbe consentire unicamente l'acquisizione di privilegi più elevati a livello locale. In Windows XP Service Pack 2, un utente anonimo non può tentare di sfruttare la vulnerabilità in remoto.
Sfruttando questa vulnerabilità, un utente malintenzionato potrebbe assumere pieno controllo di un sistema interessato, riuscendo quindi a installare programmi e visualizzare, modificare o eliminare dati oppure creare nuovi account con diritti utente completi.

**Quali sono le cause di questa vulnerabilità?**
Il processo utilizzato dal servizio Plug and Play per la convalida dei dati forniti dall'utente.
...
Sfruttando questa vulnerabilità, un utente malintenzionato autenticato potrebbe assumere pieno controllo del sistema interessato.

**In quale modo un utente malintenzionato può sfruttare questa vulnerabilità?**
In Windows 2000 e Windows XP Service Pack 1, un utente malintenzionato autenticato può tentare di sfruttare la vulnerabilità creando un messaggio di rete appositamente predisposto e inviandolo al sistema interessato. Il messaggio può quindi costringere il sistema interessato a eseguire codice. In alcune configurazioni di Windows XP, è possibile che utenti anonimi vengano autenticati come account Guest. Per ulteriori informazioni, vedere l'[advisory Microsoft sulla sicurezza 906574](http://technet.microsoft.com/security/advisory/906574). Per tentare di sfruttare questa vulnerabilità in Windows XP Service Pack 2, un utente malintenzionato deve essere in grado di accedere al sistema in locale ed eseguire un'applicazione appositamente predisposta.

**Quali sono i sistemi principalmente interessati da questa vulnerabilità?**
I sistemi principalmente a rischio sono Windows 2000 e Windows XP Service Pack 1. In Windows 2000, Windows XP Service Pack 1 e Windows XP Service Pack 2, l'utente malintenzionato deve essere in possesso di credenziali di accesso valide per sfruttare questa vulnerabilità. La vulnerabilità non può essere sfruttata in remoto da utenti anonimi in Windows 2000, Windows XP Service Pack 1 e Windows XP Service Pack 2.
**Nota** Se gli aggiornamenti per la protezione forniti con il bollettino Microsoft sulla sicurezza [MS05-039](http://technet.microsoft.com/security/bulletin/ms05-039) non sono stati installati in Windows 2000, questo problema potrebbe essere sfruttato in remoto da utenti anonimi. Se invece sono stati installati, il problema è limitato agli utenti autenticati in Windows 2000. In Windows XP Service Pack 1, questa vulnerabilità è limitata agli utenti autenticati, anche senza gli aggiornamenti per la protezione forniti con il bollettino [MS05-039](http://technet.microsoft.com/security/bulletin/ms05-039).

**È possibile sfruttare questa vulnerabilità tramite Internet?**
No, nella maggior parte dei casi. L'utente malintenzionato deve accedere al sistema specifico a cui è destinato l'attacco e Le configurazioni predefinite standard dei firewall e le procedure consigliate per la configurazione dei firewall consentono di proteggersi dagli attacchi provenienti da Internet. Microsoft fornisce informazioni su come è possibile proteggere il proprio PC. Informazioni per gli utenti finali: visitare il [sito Web Proteggi il tuo PC](http://www.microsoft.com/italy/athome/security/default.mspx). Informazioni per i professionisti IT: visitare il [sito Web Security Guidance Center](http://www.microsoft.com/italy/security/guidance/default.mspx). In Windows XP Service Pack 2, l'utente malintenzionato deve accedere al sistema specifico a cui è destinato l'attacco. Tuttavia non può caricare ed eseguire un programma in remoto come utente anonimo sfruttando questa vulnerabilità.

**Quali sono gli scopi dell'aggiornamento?**
L'aggiornamento elimina la vulnerabilità modificando il modo in cui il servizio Plug and Play convalida la lunghezza di un messaggio prima di passarlo al buffer allocato.
...
**Qual è la relazione tra questa vulnerabilità e la vulnerabilità nel servizio Plug and Play corretta dal bollettino** [**MS05-039**](http://technet.microsoft.com/security/bulletin/ms05-039)**?**
Entrambe le vulnerabilità riguardano il componente Plug and Play, ma il presente aggiornamento corregge una nuova vulnerabilità non affrontata nel bollettino [MS05-039](http://technet.microsoft.com/security/bulletin/ms05-039). [MS05-039](http://technet.microsoft.com/security/bulletin/ms05-039) consente di proteggere il sistema dalle vulnerabilità in esso illustrate, ma non da quella trattata nel presente bollettino. Questo aggiornamento sostituisce quello fornito con il bollettino [MS05-039](http://technet.microsoft.com/security/bulletin/ms05-039). È possibile installare quest'ultimo aggiornamento per proteggere il sistema in uso da entrambe le vulnerabilità.
...
Gli amministratori di sistema possono inoltre rimuovere l'aggiornamento per la protezione mediante l'utilità Spuninst.exe, disponibile nella cartella %Windir%\\$NTUninstallKB905749$\\Spuninst.
...
|--------------|---------------|-------------|-------|------------|----------|
| Umpnpmgr.dll | 5.1.2600.1734 | 23-ago-2005 | 03:51 | 111.104    | SP1QFE   |
| Umpnpmgr.dll | 5.1.2600.2744 | 23-ago-2005 | 03:35 | 123.392    | SP2GDR   |
| Umpnpmgr.dll | 5.1.2600.2744 | 23-ago-2005 | 03:39 | 123.392    | SP2QFE   |
| Arpidfix.exe | 5.1.2600.2744 | 23-ago-2005 | 01:01 | 30.720     |          |
...
-   **Controllo delle chiavi del Registro di sistema**
    ...
    **Nota** Queste chiavi del Registro di sistema potrebbero non contenere un elenco completo dei file installati. Inoltre, se un amministratore o un OEM integra o esegue lo slipstreaming dell'aggiornamento per la protezione 905749 nei file di origine dell'installazione di Windows, è possibile che queste chiavi non vengano create correttamente.
...
|--------------|---------------|--------------|-------|------------|
| Umpnpmgr.dll | 5.0.2195.7069 | 02-sett-2005 | 09:24 | 94.480     |
...
-   **Controllo delle chiavi del Registro di sistema**
    ...
    **Nota** Questa chiave del Registro di sistema potrebbe non contenere un elenco completo dei file installati. Inoltre, quando un amministratore o un OEM integra o esegue lo slipstreaming dell'aggiornamento per la protezione 905749 nei file di origine dell'installazione di Windows, è possibile che questa chiave non venga creata correttamente.
...
-   [eEye Digital Security](http://www.eeye.com/html/) per aver segnalato la vulnerabilità nel servizio Plug and Play (CAN-2005-2120).
