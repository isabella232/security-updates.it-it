---
TOCTitle: 'MS07-028'
Title: 'Bollettino Microsoft sulla sicurezza MS07-028 - Critico'
ms:assetid: 'ms07-028'
ms:contentKeyID: 61240247
ms:mtpsurl: 'https://technet.microsoft.com/it-IT/library/ms07-028(v=Security.10)'
author: SharonSears
...
Bollettino Microsoft sulla sicurezza MS07-028 - Critico
=======================================================

Una vulnerabilità in SNMP può consentire l'esecuzione di codice in modalità remota (931906)
-------------------------------------------------------------------------------------------
...
**Destinatari:** Clienti che utilizzano CAPICOM o BizTalk 2004
...
-   CAPICOM - [Download dell'aggiornamento](http://www.microsoft.com/downloads/details.aspx?displaylang=it&familyid=ca930018-4a66-4da6-a6c5-206df13af316)
-   Componente ridistribuibile GDI+ di Microsoft Platform SDK CAPICOM - [Download dell'aggiornamento](http://www.microsoft.com/downloads/details.aspx?displaylang=it&familyid=ca930018-4a66-4da6-a6c5-206df13af316)
-   BizTalk Server 2004 Service Pack 1 - [Download dell'aggiornamento](http://www.microsoft.com/downloads/details.aspx?displaylang=it&familyid=ca930018-4a66-4da6-a6c5-206df13af316)
-   BizTalk Server 2004 Service Pack 2 - [Download dell'aggiornamento](http://www.microsoft.com/downloads/details.aspx?displaylang=it&familyid=ca930018-4a66-4da6-a6c5-206df13af316)
...
| Identificatori della vulnerabilità                                                                                           | Effetti della vulnerabilità             | CAPICOM | Microsoft BizTalk Server 2004 |
|------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------|---------|-------------------------------|
| Vulnerabilità legata CAPICOM.Certificates - [CVE-2007-0940](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2007-0940) | Esecuzione di codice in modalità remota | Critico | Critico                       |
...
| Prodotto            | MBSA 1.2.1 | EST | MBSA 2.0.1 |
|---------------------|------------|-----|------------|
| CAPICOM             | No         | Sì  | Sì         |
| BizTalk Server 2004 | No         | Sì  | Sì         |

Per ulteriori informazioni su MBSA, visitare il [sito Web MBSA](http://www.microsoft.com/technet/security/tools/mbsahome.mspx). Per ulteriori informazioni sul software che attualmente non viene rilevato da Microsoft Update e MBSA 2.0, vedere l'[articolo della Microsoft Knowledge Base 895660](http://support.microsoft.com/kb/895660).
...
|---------------------|--------------|----------|
| CAPICOM             | Sì (con EST) | Sì       |
| BizTalk Server 2004 | Sì (con EST) | Sì       |
...
Sì. È possibile utilizzare SMS per determinare se nel sistema sono presenti programmi che potrebbero avere installato una propria versione del componente affetto dalla vulnerabilità. SMS può verificare se il file CAPICOM.dll è presente nel sistema. Aggiornare tutte le versioni del file CAPICOM.dll precedenti alla versione 2.1.0.2.

Dettagli della vulnerabilità
----------------------------

<span></span>
#### Vulnerabilità legata a CAPICOM.Certificates - CVE-2007-0940:

In CAPICOM (Cryptographic API Component Object Model) esiste una vulnerabilità associata all'esecuzione di codice in modalità remota che potrebbe consentire a un utente malintenzionato in grado di sfruttare questa vulnerabilità di assumere il controllo completo di un sistema interessato.

#### Fattori attenuanti della vulnerabilità legata a CAPICOM.Certificates - CVE-2007-0940:
...
#### Soluzioni alternative per la vulnerabilità legata a CAPICOM.Certificates - CVE-2007-0940:

Microsoft ha verificato le soluzioni alternative seguenti. Sebbene queste soluzioni alternative non eliminino la vulnerabilità sottostante, consentono di bloccare gli attacchi noti. Se una soluzione alternativa riduce la funzionalità, viene identificata nella sezione seguente.

-   **Disattivare i tentativi di creare l'istanza CAPICOM in Internet Explorer**

    È possibile disattivare i tentativi di creare un'istanza di questo controllo ActiveX impostando il kill bit per il controllo nel Registro di sistema.
    ...
    **Avvertenza** L'utilizzo non corretto dell'Editor del Registro di sistema potrebbe causare problemi tali da rendere necessaria la reinstallazione del sistema operativo. Microsoft non garantisce la risoluzione di problemi dovuti a un utilizzo non corretto dell'Editor del Registro di sistema. È quindi necessario utilizzare questo strumento con grande attenzione.
    ...
    Per impostare il kill bit per i controlli CLSID con valore
    ...
    **Effetto della soluzione**: Il controllo CAPICOM potrebbe non essere più visualizzato o funzionare correttamente.
...
#### Domande frequenti sulla vulnerabilità legata a CAPICOM.Certificates - CVE-2007-0940:
...
Il modo in cui la classe CAPICOM.Certificates gestisce alcuni input causa la vulnerabilità. Se vengono forniti dati imprevisti, il controllo ActiveX può provocare un errore tale da consentire l'esecuzione di codice in modalità remota.

**Che cos'è CAPICOM.Certificates? **
CAPICOM.Certificates è un controllo ActiveX che fornisce ai creatori di script (VBS, ASP, ASP.NET, ecc.) un metodo per la crittografia dei dati basati sulla funzionalità protetta Windows CryptoAPI. La suite CAPICOM è anche disponibile per il download come [Platform SDK Redistributable: CAPICOM](http://www.microsoft.com/downloads/details.aspx?familyid=860ee43a-a843-462f-abb5-ff88ea5896f6) ed, inoltre, è parte di Windows Platform SDK e di Windows Driver Kit.

**Cosa accade se si sviluppano applicazioni o software e si ridistribuisce CAPICOM? **
È necessario scaricare la versione più recente di [Platform SDK Redistributable: CAPICOM](http://www.microsoft.com/downloads/details.aspx?familyid=860ee43a-a843-462f-abb5-ff88ea5896f6). Questa nuova versione contiene il file CAPICOM.dll aggiornato relativo all'aggiornamento di protezione.

**Come è possibile stabilire se CAPICOM.Certificates è già installato e registrato sul sistema? **
Per verificare se CAPICOM è già installato sul sistema, è possibile effettuare una ricerca per CAPICOM.dll. Se si dispone della versione 2.1.01 o precedente, è necessario aggiornare il sistema.

È possibile verificare la presenza di una delle seguenti combinazioni di chiavi del Registro di sistema per sapere se si sul proprio sistema è presente una versione registrata del controllo ActiveX CAPICOM.Certificates:
...
**Perché la vecchia versione di CAPICOM.dll è ancora presente nel sistema dopo aver installato questo aggiornamento per la protezione? **
È possibile che gli utenti abbiano installato CAPICOM.dll da un'origine diversa da Microsoft. CAPICOM.dll può essere stato installato da un prodotto non-Microsoft. Poiché l'aggiornamento per la protezione non può determinare da dove proviene la versione di CAPICOM.dll precedente, questa viene lasciata nel sistema per garantire la compatibilità tra applicazioni. La versione precedente non espone il sistema a questa vulnerabilità.

**Quali capacità sono necessarie per sfruttare questa vulnerabilità? **
In uno scenario di attacco basato sul Web, l'utente malintenzionato dovrebbe pubblicare un sito Web contenente una pagina Web da utilizzare per sfruttare la vulnerabilità. Poiché non è in alcun modo possibile obbligare gli utenti a visitare un sito Web appositamente predisposto, l'utente malintenzionato dovrebbe convincere le vittime a visitare il sito Web, in genere inducendole a fare clic su un collegamento che le indirizzi al sito.

**Quali sono i sistemi principalmente interessati da questa vulnerabilità? **
Questa vulnerabilità può essere sfruttata per l'esecuzione di azioni dannose solo se un utente è connesso al sistema e visita un sito Web. Pertanto, i sistemi maggiormente a rischio sono quelli nei quali è installato e registrato CAPICOM Certificates e dove Internet Explorer viene utilizzato frequentemente, come le workstation o i server terminal.

**L'esecuzione di Internet Explorer 7 attenua l'impatto di questa vulnerabilità? **
Sì. I clienti che eseguono Internet Explorer 7 con le impostazioni predefinite non sono esposti finché non viene attivato il controllo CAPICOM Certificates tramite la funzione ActiveX con consenso esplicito nell'area Internet. Comunque, se un cliente ha utilizzato questo controllo Activex in una versione precedente di Internet Explorer, il controllo Activex è attivato per funzionare in Internet Explorer 7, anche se il cliente non l'ha approvato esplicitamente utilizzando la funzione Activex con consenso esplicito.
...
L'aggiornamento elimina la vulnerabilità modificando il modo in cui il controllo ActiveX CAPICOM.Certificates esegue la convalida dei parametri. Inoltre, risolve altri problemi scoperti tramite ricerche interne.
...
**Prerequisiti**
Questo aggiornamento per la protezione richiede l'installazione e la registrazione di CAPICOM Certificates. BizTalk Server 2004 installa CAPICOM Certificates. L'aggiornamento funziona sia per le installazioni autonome che di BizTalk Server 2004. I prodotti non-Microsoft possono ridistribuire e installare CAPICOM.dll. Per ulteriori informazioni, vedere la domanda frequente "**Come è possibile stabilire se CAPICOM.Certificates è già installato e registrato sul sistema?"**.

**Disponibilità nei service pack futuri**
L'aggiornamento per il problema illustrato verrà incluso in un service pack o rollup di aggiornamento futuro per Microsoft Biztalk Server 2004.
...
**Nota** Queste opzioni non funzionano necessariamente con tutti gli aggiornamenti. Se un'opzione non è disponibile, la funzionalità è necessaria per installare l'aggiornamento in modo corretto. Se l'installazione non viene eseguita correttamente, consultare il supporto tecnico per individuare il problema.
...
|-------------|----------|-------------|-------|------------|
| License.mht | N/D      | 26-feb-2007 | 23:16 | 142.534    |
| License.rtf | N/D      | 26-feb-2007 | 23:16 | 134.577    |
...
-   Chris Ries di [VigilantMinds Inc.](http://www.vigilantminds.com/) per aver segnalato la vulnerabilità legata a CAPICOM.Certificates (CVE-2007-0940)
